...
public class DMSTGraph extends Graph {
	...
	GraphHash<List<DMSTVertex>, DMSTEdge> gh = new GraphHash<List<DMSTVertex>, DMSTEdge>(g);
	...
	public DMSTGraph(Graph g, Vertex start, List<Edge> dmstEdges) {
		...
		this.source = new DMSTVertex(start);
		this.g = g;
	...
	class DMSTIterator implements Iterator<Vertex> {
		...
		DMSTIterator(DMSTGraph dm) {
			this.it = new ArrayIterator<DMSTVertex>(dm.dmst, 0,
					noOfVertices - 1); // Iterate
										// over
										// existing
										// elements
										// only
	...
	private void storeComponentVertices() {
		...
		for (Vertex v : this) {
			...
			if (componentVertices.containsKey(dv.getComponentNumber())) {
				List<DMSTVertex> vertexComponents = componentVertices.get(dv
						.getComponentNumber());
				vertexComponents.add(dv);

				componentVertices
						.put(dv.getComponentNumber(), vertexComponents);
			}
		...
		for (Map.Entry<Integer, List<DMSTVertex>> tempMap : componentVertices
				.entrySet()) {
	...
	private void shrinkComponents(
			HashMap<Integer, List<DMSTVertex>> componentVertices) {
		...
		for (DMSTEdge[] row : minEdgesBetweenComponents) {
			for (DMSTEdge column : row) {
				...
				List<DMSTVertex> cfVertices = componentVertices.get(from
						.getComponentNumber());
				List<DMSTVertex> ctVertices = componentVertices.get(to
						.getComponentNumber());
				...
				if (cfVertices.size() > 1) {
					...
					compVertex[from.getComponentNumber() - 1] = newVertex[from
					...
					compVertex[from.getComponentNumber() - 1] = cfVertices
							.get(0);
				}
				...
				if (ctVertices.size() > 1) {
					// Vertex for each component
					compVertex[to.getComponentNumber() - 1] = newVertex[to
							.getComponentNumber() - 1];
	...
	// private void updateMinimumWeightMatrix(int compNo, DMSTVertex
	// componentVertex,DMSTEdge cEdge) {
	//
	// DMSTEdge edge= new
	// DMSTEdge((DMSTVertex)cEdge.toVertex(),componentVertex,cEdge.tempWeight);
	//
	...
	//
	// }
	// private void createNewEdges(DMSTVertex fromVertex, DMSTVertex toVertex,
	// int weight) {
	//
	...
	private void findMinimumEdgeBetweenComponents(int noOfComponents) {
		...
		for (Vertex v : this) {
			...
			if (dv.getName() == source.getName()) {
				minEdgesBetweenComponents[dv.getComponentNumber() - 1][dv
						.getComponentNumber() - 1] = null; // for
				// root
				// node
			}
			...
			for (Edge e : dv) {
				...
				if (du.getComponentNumber() == dv.getComponentNumber()) {
					...
					if (minEdgesBetweenComponents[du.getComponentNumber() - 1][dv
							.getComponentNumber() - 1] != null) {
						minEdge = minEdgesBetweenComponents[du
								.getComponentNumber() - 1][dv
								.getComponentNumber() - 1];

						if (tempEdge.tempWeight <= minEdge.tempWeight) {
							minEdge = tempEdge;
							minEdgesBetweenComponents[du.getComponentNumber() - 1][dv
									.getComponentNumber() - 1] = minEdge;
						}
					}
	...
	private boolean bfsFindMST() {
		...
		BFS b = new BFS(this, dmst[source.getName()]);
