 ...
 * 
 * @Description Version of binary search that returns index of the largest
 *              element that is less than or equal to x
 */

public class BinarySearch {

	public static <T extends Comparable<? super T>> int recursiveBinarySearch(
			T[] A, int p, int r, T x) {
		...
		if (p <= r) {
			...
			if (cmp < 0) { // A[q] is too small, x is not in left half
				...
				if (q!=A.length-1 && A[q + 1].compareTo(x) > 0) {
					return q;
				} else if (q!=A.length-1 && A[q + 1].compareTo(x) == 0) {
					return q + 1;
				} else {
						if(q+1==A.length)
						{
